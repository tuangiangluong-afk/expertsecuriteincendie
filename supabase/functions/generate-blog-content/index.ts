    ...
    try {
        ...
        switch (action) {
            case "fetch_trends": {
                ...
                let queries = ["matériel incendie avis", "maintenance Incendie prix"];
                try {
                    const clean = resultText.replace(/```json/g, "").replace(/```/g, "").trim();
            ...
            case "write_article": {
                ...
                ${landingPages?.map(p => `- Keyword: "${p.h1_title}" or "${p.sport} ${p.city}" -> URL: /maintenance-extincteur-${p.city.toLowerCase()}`).join('\n') || "No landing pages."}
