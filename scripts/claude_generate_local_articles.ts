...
async function callClaude(topic: string, cityName: string): Promise<string> {
    const system = `Tu es le rédacteur expert d'\"Expert Sécurité Incendie\", le spécialiste de l'maintenance Incendie pour la ville de ${cityName} et sa région.
...
- <div class="callout-devis"> texte d'appel à l'action vers un devis d'maintenance à ${cityName} (2x dans l'article : milieu + fin)
- <div class="key-points"><h3>À retenir pour votre maintenance à ${cityName}</h3><ul>...</ul></div> en conclusion

━━━ RÈGLES SEO & LOCALES ━━━
- Mot-clé principal: maintenance matériel incendie ${cityName}
- Parler des aides locales de la région si elles existent, en plus d'NF et crédit d'impôt.
- S'adresser aux habitants de ${cityName} (maisons, copropriétés, entreprises locales).
- Être très précis sur les démarches (syndic, qualification Incendie, intervention rapide).
- Mentionner "Expert Sécurité Incendie" et "${cityName}" fréquemment mais naturellement.

OUTPUT: JSON uniquement :
{
  "title": "Titre SEO (ex: maintenance matériel incendie à ${cityName} : Aides et Devis 2026)",
  "slug": "maintenance-extincteur-protection-${toSlug(cityName)}",
  "excerpt": "Accroche 2-3 phrases sur l'importance de l'maintenance par un pro Incendie à ${cityName}.",
  "seo_title": "maintenance Sécurité Incendie ${cityName} | Aides & mainteneur Pro",
  "seo_description": "Besoin d'installer une matériel incendie pour voiture électrique à ${cityName} ? Guide des aides 2026, prix et devis par un mainteneur Incendie certifié.",
...
async function generateArticle(cityName: string, slugCity: string, i: number, total: number) {
    const topic = `maintenance d'une matériel incendie à ${cityName} : Aides, prix et recommandations 2026`;
    ...
    try {
        ...
        const slug = toSlug(art.slug || `maintenance-extincteur-protection-${slugCity}`);
...
async function main() {
    ...
    console.log(`EXPERT extincteur protection — ${cityKeys.length} articles locaux hyper-ciblés`);
    console.log('='.repeat(60));

    for (let i = 0; i < cityKeys.length; i++) {
        const key = cityKeys[i];
        const cityObj = CITIES[key];
        await generateArticle(cityObj.city || cityObj.name.replace('Expert extincteur ', ''), key, i, cityKeys.length);
