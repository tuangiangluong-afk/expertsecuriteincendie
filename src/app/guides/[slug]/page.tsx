...
const components = {
    ...
    Callout: ({ children, title }: { children: React.ReactNode, title?: string }) => (
        <div className="my-8 border-l-4 border-red-600 bg-red-50 p-6 rounded-r-xl">
            {title && <h4 className="font-bold text-red-900 mb-2 flex items-center gap-2"><Zap size={18} /> {title}</h4>}
    ...
    CtaButton: ({ text, url }: { text: string, url: string }) => (
        <div className="my-8 text-center not-prose">
            <a href={url} className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-8 rounded-full shadow-lg hover:shadow-blue-500/30 transition-all transform hover:-translate-y-1">
...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const articleSchema = {
        ...
        "author": {
            "@type": "Organization",
            "name": guide.meta.author || "Expert Sécurité Incendie"
        },
        "publisher": {
            "@type": "Organization",
    ...
    return (
        <div className="min-h-screen bg-white text-slate-900 font-sans">
            ...
            <main className="container mx-auto px-4 py-12 pt-40">
                ...
                <div className="grid lg:grid-cols-[1fr_350px] gap-12 max-w-7xl mx-auto">
                    ...
                    <div>
                        {/* Header Article */}
                        <div className="mb-10">
                            <Link href="/guides" className="inline-flex items-center text-sm text-slate-500 hover:text-red-600 mb-6 group">
                        ...
                        {/* Content Body */}
                        <article className="prose prose-lg prose-slate prose-headings:font-bold prose-headings:text-slate-900 prose-headings:scroll-mt-32 prose-a:text-red-600 hover:prose-a:text-red-700 prose-img:rounded-2xl max-w-none">
                            {dbPost ? (
                        ...
                        <div className="mt-16 p-8 bg-slate-50 rounded-2xl border border-slate-200">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <div className="font-bold text-lg text-slate-900">Expert Sécurité Incendie</div>
                                    <div className="text-sm text-slate-500">Pôle Technique & Réglementation</div>
                                </div>
                            </div>
                            <p className="text-slate-600 mb-4">
                                Nos guides sont rédigés par des experts en sécurité incendie pour vous aider à comparer les meilleures solutions du marché. Les informations sur les aides (NF, Crédit d&apos;Impôt) sont vérifiées régulièrement auprès des organismes officiels.
                            </p>
                        </div>
                    </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur border-t border-slate-200 lg:hidden z-40">
                <a href="/#simulateur" className="flex items-center justify-center gap-2 w-full bg-red-600 text-white font-bold py-3 rounded-xl shadow-lg">
                    <Zap size={18} />
