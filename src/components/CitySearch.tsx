    ...
    return (
        <div ref={wrapperRef} className="relative w-full max-w-2xl mx-auto">
            <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-neutral-400 group-focus-within:text-red-500 transition-colors">
                    <Search size={20} />
                </div>
                <input
                    type="text"
                    className="w-full pl-12 pr-4 py-5 bg-white rounded-2xl border-2 border-transparent focus:border-red-500 shadow-xl text-lg font-medium outline-none transition-all placeholder:text-neutral-400 text-neutral-900"
                    ...
                    <div className="py-2">
                        {results.map((result) => (
                            <button
                                ...
                                className="w-full text-left px-5 py-3 hover:bg-red-50 transition flex items-center gap-4 group"
                            >
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${result.type === 'CITY' ? 'bg-red-100 text-red-600' : 'bg-orange-100 text-orange-600'
                                    }`}>
                                    {result.type === 'CITY' ? <MapPin size={20} /> : <Train size={20} />}
                                </div>
                                <div className="flex-1">
                                    <div className="font-bold text-neutral-900 group-hover:text-red-700">{result.label}</div>
                                    <div className="text-xs text-neutral-500">{result.desc}</div>
                                </div>
                                <ArrowRight size={16} className="text-neutral-300 group-hover:text-red-500 -translate-x-2 group-hover:translate-x-0 transition-all opacity-0 group-hover:opacity-100" />
                            </button>
                        ))}
                    </div>
                </div>
